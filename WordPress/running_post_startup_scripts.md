...
WordPress on Linux App Service offering is based on alpine linux distro. You can use the default **apk add _ _/<packagename/>_ _** command to install required packages or package manager. The following command uses pecl to install imagick library for PHP
